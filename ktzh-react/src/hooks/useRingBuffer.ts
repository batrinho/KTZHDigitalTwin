...
const MAX_SIZE = 900; // 15 min × 60 s/min = 900 data points
...
export class RingBuffer {
  ...
  getLast(n: number): TimestampedTelemetry[] {
...
export function useRingBuffer() {
  ...
  const push = useCallback((entry: TimestampedTelemetry) => {
